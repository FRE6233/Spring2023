...
namespace fms::carr_madan {
	...
	// Replace f with delta function weights (f[0], (f[1] - f[0])/(k[1] - k[0]), f''(k[1]), ..., f''(k[n - 2]))
	template<class X = double, size_t N>
	...
	{
		{
			double k[] = { 0, 1, 2, 3 }, f[] = { 0, 1, 1, 0 };
			auto fs = std::span(f); // l-value
			fit(std::span(k), fs);
			//ensure(f == std::vector<double>({0, 1, -1, -1}));
			unfit(std::span(k), fs);
			//ensure(f == std::vector<double>({ 0, 1, 1, 0 }));
		}
	...
	// price pwlinear k, f, using price x, puts below a, and calls above a
	template<class X = double, size_t N>
	inline constexpr X value(const X& x, const X& a, const std::span<X,N>& p, const std::span<X,N>& c,
		const std::span<X,N>& k, std::span<X,N>& f)
	{
		size_t n = p.size();
		ensure(n == c.size());
		ensure(n == k.size());
